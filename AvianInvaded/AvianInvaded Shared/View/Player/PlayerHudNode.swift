...
class PlayerHudNode: SKNode {
    ...
    init(sceneSize: CGSize) {
        lifeBar = .init(rectOf: CGSize(width: sceneSize.width*0.4, height: sceneSize.height*0.05))
        lifeBar.fillColor = SKColor.red.withAlphaComponent(0.7)
        lifeBar.strokeColor = .clear
        
        lostLifeBar = .init(rectOf: CGSize(width: sceneSize.width*0.4, height: sceneSize.height*0.05))
        lostLifeBar.fillColor = SKColor.darkGray.withAlphaComponent(0.7)
        lostLifeBar.strokeColor = .clear
        
        let rect = CGRect(x: -(sceneSize.width*0.90)/2, y: -sceneSize.height*0.01, width: sceneSize.width*0.90, height: sceneSize.height*0.01)
        
        emptyXpBar = .init(rect: rect, cornerRadius: sceneSize.height*0.01)
        emptyXpBar.fillColor = SKColor.gray.withAlphaComponent(0.7)
        emptyXpBar.strokeColor = .clear
        
        xpBar = .init(rect: rect, cornerRadius: sceneSize.height*0.01)
        ...
        self.zPosition = 11
        self.addChildren([lostLifeBar, lifeBar, emptyXpBar, xpBar])
    }
